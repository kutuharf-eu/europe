...
// ── Form kutusu (logo / çubuk) sentetik katsayısı ────────────────────────────
// Tek "harf" X = formun tamamı — eCut geometrik form tablosuyla birebir (23 Tem 2026):
//   rect   → çevre = 2(en+boy) (kare 4a, çubuk 2(l+w) doğrulandı)
//   circle → çevre = π(en+boy)/2 (elips; en=boy → πd — xlsx Ø50: 1,571 m ✓)
// Plaka HER şekilde en×boy: yuvarlak da kare plakadan kesilir (xlsx "50 x 50 (Ø50)").
const formCoeffs = (wM, hM, shape) => ({
  font: shape === 'circle' ? 'circle' : 'rect',
  chars: { X: {
    alan: shape === 'circle' ? (Math.PI * wM * hM) / 4 / (hM * hM) : wM / hM,
    cevre: (shape === 'circle' ? (Math.PI * (wM + hM)) / 2 : 2 * (wM + hM)) / hM,
    genislik: wM / hM,
    yukseklik: 1,
  } },
});

// LED alanı (m²): daire formda gerçek daire/elips alanı (πwh/4 — xlsx Ø50: 25 LED ✓),
// dikdörtgende en×boy. LED aydınlatılan yüzeyi izler, plakayı değil.
const ledAreaM2 = (wM, hM, shape) => (shape === 'circle' ? (Math.PI * wM * hM) / 4 : wM * hM);

// Çubuk LED yoğunluğu (eCut xlsx çubuk serisi): 5 cm yükseklik tek sıra ≈ 9,33 ad/m
// (14/1,5m · 28/3m); her ek cm yükseklik ≈ +0,7 ad/m (10 cm satırları ~12,7–14 ad/m).
const CUBUK_LED_PER_M = 9.33;
const CUBUK_LED_ROW_STEP = 0.7;

// ── Font → katsayı dosyası eşlemesi ──────────────────────────────────────────
// eCut mantığı: her font için GERÇEK glyph geometrisi kullanılır. Katsayılar tüm
// konfigüratör fontları için üretildi (npm run coeff:all — konfigüratördeki aynı
// Google kesimi/ağırlıkla). Dosyası olmayan font (ör. custom upload) için eski
// yaklaşıklık: el yazıları → oswald, kondanse → anton, geniş/kalın → archivo.
export function coeffFontFor(fontId) {
  const f = KONFIG_FONTS.find((x) => x.id === fontId);
  if (f && !f.custom && existsSync(path.join(process.cwd(), 'public', 'coefficients', `${fontId}.json`))) {
    return fontId; // gerçek font geometrisi (eCut eşdeğeri)
  }
  if (!f) return 'archivo';
  if (f.cat === 'schrift') return 'oswald';
  if (f.widthFactor <= 0.52) return 'anton';
  return 'archivo'; // custom + katsayısı üretilmemiş fontlar
}
...
async function motorKonfigPrice(cfg, vars, extras, opts = {}) {
  ...
  // Form (logo/çubuk) montaj gideri, harfin aksine LED adediyle BÜYÜR (25 Tem 2026
  // üretici föyü: 11 kare/yuvarlak/çubuk satırı → sabit ≈ 166,5 + 2,74 × LED, ±12 TL).
  // Motorun kalem toplamı 147 TL olduğu için taban `formMontajTaban` (≈19,5 TL) ile,
  // LED'e bağlı kısım da bu fonksiyonla eklenir. Harf tarafı düz `montajGiderPerHarf`.
  const formMontajLedTRY = (ledAdet, w) => toTRY(vars.formMontajPerLed, vars, w, 'Form montaj (LED başına)') * ledAdet;

  // UV Baskı: yalnız önden akrilik ürünlerde (ön yüz = pleksi_oto) uygulanır. Müşteri
  // seçse bile front akrilik değilse motor uygulamaz — gerçeklik/fiyat kapısı burada.
  const uvBaski = cfg.uvBaski === true && model.surfaces.face === 'pleksi_oto';
  const cost = computeCost({
    text: cfg.text, h, model, depth: depthM(cfg.depth, cfg.heightCm),
    ledType: 'standart', kromBoyama, uvBaski, vars, coeffs,
  });
  ...
  // Harfsiz ürün (sadece Logo/Çubuk LED): N=0 kabul — logo/çubuk kendi bloğunda fiyatlanır.
  const hasComponent = !!(cfg.logo || cfg.cubukLed);
  if (cost.geometry.missing.length) return null;
  if (!cost.geometry.N && !hasComponent) return null;
  const surfaceBroken = cost.items.some((i) => (i.birim === 'm²' || i.birim === 'mt') && i.miktar > 0 && i.tryTotal <= 0);
  // Harfsiz üründe (N=0) LED kalemi 0 TL'dir (harf yok) — bu bozukluk değil. Yalnız harf varken kontrol et.
  const ledBroken = lit && cost.geometry.N > 0 && cost.items.some((i) => i.id === 'led' && i.tryTotal <= 0);
  ...
  const extraTRY = extraCostsTRY({ extras, eff, constructionId: cfg.constructionId, N, areaM2: cost.geometry.toplamPlaka, prodTRY, vars, warnings });
  ...
  const perLetter = N > 0 ? round2(lettersTotal / N) : 0;
  ...
  // addon (bağımsız ek ürün): trafo da proje-seviyesi — tek tabela için tek trafo ana
  // Harf kalemi üzerinden alınır; ek logo/çubuk kalemi kendi trafosunu taşımaz.
  let netzteil = 0;
  if (lit && cfg.trafo !== false && !opts.addon) {
  ...
  if (cfg.logo) {
    ...
    if (!lg) return null;
    if (cfg.logoPrint === 'uv') {
      // Baskı logo (UV): düz baskı → m² × baskı fiyatı (admin €/m²). Harf/kutu
      // mantığı YOK. Fiyat doğrudan alan × birim (markup uygulanmaz — admin fiyatı satıştır).
      const areaM2 = (lg.widthCm / 100) * (lg.heightCm / 100);
      const unitTRY = toTRY(vars.logoUvBaskiM2, vars, warnings, 'UV baskı logo');
      if (unitTRY <= 0) return null; // baskı fiyatı tanımsız → legacy'ye düş
      logoCostTRY = areaM2 * unitTRY * fire;
      logoInfo = { ...lg, print: cfg.logoPrint, areaM2: Math.round(areaM2 * 100) / 100, total: round2(eur(areaM2 * unitTRY)) };
    } else {
      // 3D logo (23 Tem 2026 eCut): kutu GERÇEK geometrisiyle fiyatlanır — plaka =
      // en×boy; çevre + LED alanı şekle göre (rect / circle, bkz. formCoeffs). LED alan
      // bazlıdır (ledYogunlugu ≈ 127 ad/m², xlsx kare+yuvarlak serisi birebir).
      // eq artık YALNIZ işçilik kalemini ölçekler.
      const eq = logoEquivalentLetters(lg.widthCm, lg.heightCm);
      const wM = lg.widthCm / 100, hM = lg.heightCm / 100;
      const ledAdet = Math.ceil(ledAreaM2(wM, hM, lg.shape) * (vars.ledYogunlugu ?? 127));
      const lCost = computeCost({
        text: 'X', h: hM, model, depth: depthM(cfg.depth, lg.heightCm),
        // Logo UV, harfin malzemesinden BAĞIMSIZ: logo ayrı bir akrilik baskılı kutu,
        // harf krom olsa bile logoya UV baskı yapılabilir. Ücret = logo plaka alanı × birim.
        ledType: 'standart', kromBoyama, uvBaski: cfg.logoUv === true,
        vars: { ...vars, ledAdetSabit: ledAdet, montajGiderPerHarf: vars.formMontajTaban },
        coeffs: formCoeffs(wM, hM, lg.shape),
      });
      const lProd = lCost.items.filter((i) => i.id !== 'trafo').reduce((a, i) => a + i.tryTotal, 0);
      logoCostTRY = (lProd + formMontajLedTRY(ledAdet, warnings) + iscilikUnitTRY * eq) * fire;
      const logoTotal = round2(eur(logoCostTRY * risk * marj));
      logoInfo = { ...lg, eqLetters: eq, perLetter: round2(logoTotal / eq), total: logoTotal };
    }
  }

  // Çubuk LED (LED-Leiste, 23 Tem 2026 eCut): GERÇEK dikdörtgen geometri — plaka =
  // uzunluk×yükseklik, çevre = 2(l+h). LED metre bazlı sıra mantığı (CUBUK_LED_PER_M).
  // Parça listesi (max 150 cm) yalnız gösterim içindir, fiyat toplam ölçüden hesaplanır.
  let cubukInfo = null;
  let cubukCostTRY = 0;
  if (cfg.cubukLed) {
    ...
    const lM = cl.lengthCm / 100, hM = cl.heightCm / 100;
    const ledAdet = Math.ceil(lM * (CUBUK_LED_PER_M + CUBUK_LED_ROW_STEP * Math.max(0, cl.heightCm - 5)));
    const cCost = computeCost({
      text: 'X', h: hM, model, depth: depthM(cfg.depth, cl.heightCm),
      // Çubuk UV, harfin malzemesinden BAĞIMSIZ (logo gibi): çubuk ayrı akrilik ön yüzlü
      // bir eleman, harf krom olsa bile çubuğa UV baskı yapılabilir. Ücret = plaka alanı × birim.
      ledType: 'standart', kromBoyama, uvBaski: cfg.cubukUv === true,
      vars: { ...vars, ledAdetSabit: ledAdet, montajGiderPerHarf: vars.formMontajTaban },
      coeffs: formCoeffs(lM, hM, 'rect'),
    });
    const cProd = cCost.items.filter((i) => i.id !== 'trafo').reduce((a, i) => a + i.tryTotal, 0);
    cubukCostTRY = (cProd + formMontajLedTRY(ledAdet, warnings) + iscilikUnitTRY * eq) * fire;
    const cubukTotal = round2(eur(cubukCostTRY * risk * marj));
    cubukInfo = { ...cl, pieces: cubukLedPieces(cl.lengthCm), eqLetters: eq, perLetter: round2(cubukTotal / eq), total: cubukTotal };
  }

  // Sipariş bazlı: ambalaj (risk+marj ile) + minimum sipariş tabanı (montaj hariç uygulanır)
  // addon (bağımsız ek ürün: ayrı sepete eklenen logo / çubuk kalemi): proje-seviyesi
  // ücretler UYGULANMAZ — ambalaj, minimum sipariş ve montaj sepette ana Harf kalemi
  // üzerinden bir kez alınır. Ek ürün yalnız kendi üretim maliyeti + kendi trafosunu taşır.
  // zusatz (ek yazı bloğu, çok yazılı proje): aynı sipariş içinde 2..n. yazı. Proje-seviyesi
  // ücretler burada da UYGULANMAZ (ilk yazı kaleminde bir kez alındı), ama addon'dan farklı
  // olarak KENDİ trafosunu taşır — farklı font/tabela tipi ayrı bir üretim işidir.
  const projektFrei = opts.addon || opts.zusatz;
  const ambalaj = projektFrei ? 0 : round2(eur(toTRY(vars.ambalajTaban, vars, warnings, 'Ambalaj') * risk * marj));
  const minEUR = projektFrei ? 0 : (vars.minSiparis?.currency === 'EUR' ? Number(vars.minSiparis.amount) || 0 : 0);
  const montagePrice = projektFrei ? 0 : montage.price;
  ...
  const total = round2(Math.max(prodTotal, minEUR) + montagePrice);

  const result = { letters: N, perLetter, lettersTotal, letterRows, logo: logoInfo, cubukLed: cubukInfo, netzteil, ambalaj, minApplied, montage: montagePrice, total, source: 'motor' };
  ...
  if (opts.detailed) {
    ...
    // Ek yazı bloğu (zusatz) / bağımsız ek ürün (addon): ambalaj ve montaj proje
    // seviyesindedir, ilk kalemde sayıldı → maliyet tarafında da ikinci kez sayılmaz,
    // yoksa admin kıyas tablosunda çok yazılı projenin maliyeti şişer.
    const ambalajCostTRY = projektFrei ? 0 : toTRY(vars.ambalajTaban, vars, warnings, 'Ambalaj');
    const lettersCostTRY = (prodTRY + extraTRY + iscilikTRY) * fire;
    const costOrderTRY = lettersCostTRY + trafoCostTRY + ambalajCostTRY + logoCostTRY + cubukCostTRY + montagePrice * vars.eurTry;
    const costOrderEUR = round2(eur(lettersCostTRY + trafoCostTRY + ambalajCostTRY + logoCostTRY + cubukCostTRY) + montagePrice);

    // ── SALT MALZEME (25 Tem 2026, Murat'ın tanımı) ────────────────────────────
    // Ele giren fiziksel malzemenin tamamı: yüzeyler (pleksi/dekota/alu/krom/rakı)
    // + UV baskı + LED + kablo + vida + yapıştırıcı + halo ayağı + krom boyama.
    // HARİÇ: trafo, montaj gideri, ambalaj, nakliye/montaj, işçilik, admin ek
    // kalemleri (extras), risk ve marj. Fire dahildir (gerçek tüketim).
    const MALZEME_DISI = new Set(['trafo', 'montaj_gideri']);
    const malzemeTRY = cost.items.reduce((a, i) => (MALZEME_DISI.has(i.id) ? a : a + i.tryTotal), 0) * fire;

    result._detail = {
      eurTry: vars.eurTry, fire, risk, marj, marjKey,
      costOrderEUR, costOrderTRY: round2(costOrderTRY),
      malzemeEUR: round2(eur(malzemeTRY)), malzemeTRY: round2(malzemeTRY),
      breakdown: {
...
// opts.zusatz: çok yazılı örnekte 2..n. yazı — ambalaj/minimum/montaj ikinci kez
// sayılmaz (müşteri tarafındaki `zusatz` ile birebir aynı kural).
export async function serverAdminPricePreview(cfg, opts = {}) {
  const legacy = konfigPrice(cfg, opts);
  ...
  // Händler kademeleri de hesaplanır (25 Tem 2026): admin tablosunda Vielverkäufer /
  // Wenigverkäufer satırları Standart ve Premium ile aynı şekilde görünsün — bunlar
  // canlıda gerçekten uygulanan kademeler (resolveHaendler → marjKey), kör nokta kalmasın.
  const tierKeys = ['rekabetci', 'haendler_cok', 'haendler_az', 'standart', 'premium'];
  const tiers = {};
  let cost = null;
  let letters = 0;
  for (const tk of tierKeys) {
    const p = await motorKonfigPrice(cfg, vars, extras, { marjKey: tk, detailed: true, zusatz: opts.zusatz === true });
    if (!p || !p._detail) return { ok: false, reason: 'legacy' };
    if (!cost) {
      cost = {
        orderEUR: p._detail.costOrderEUR,
        orderTRY: p._detail.costOrderTRY,
        // Salt malzeme: ele giren fiziksel malzeme (trafo/ambalaj/nakliye/montaj gideri hariç)
        materialEUR: p._detail.malzemeEUR,
        materialTRY: p._detail.malzemeTRY,
...
export async function serverKonfigPrice(cfg, opts = {}) {
  const legacy = konfigPrice(cfg, opts);
  ...
  if (vars) {
    try {
      const p = await motorKonfigPrice(cfg, vars, extras, opts);
