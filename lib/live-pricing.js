...
async function motorKonfigPrice(cfg, vars, extras, opts = {}) {
  ...
  if (cfg.logo) {
    ...
    if (cfg.logoPrint === 'uv') {
      ...
      // 3D logo: logo yüksekliğinde "ortalama harf" × eşdeğer harf sayısı — kutu mantığı
      ...
      const lCost = computeCost({
        ...
        // harf krom olsa bile logoya UV baskı yapılabilir. Ücret = logo ön yüz alanı × birim.
        ledType: 'standart', kromBoyama, uvBaski: cfg.logoUv === true, vars, coeffs: synth,
      });
  ...
  if (cfg.cubukLed) {
    ...
    const cCost = computeCost({
      ...
      // bir eleman, harf krom olsa bile çubuğa UV baskı yapılabilir. Ücret = ön yüz alanı × birim.
      ledType: 'standart', kromBoyama, uvBaski: cfg.cubukUv === true, vars, coeffs: synth,
    });
