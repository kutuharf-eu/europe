...
async function motorKonfigPrice(cfg, vars, extras, opts = {}) {
  ...
  const ambalaj = opts.addon ? 0 : round2(eur(toTRY(vars.ambalajTaban, vars, warnings, 'Ambalaj') * risk * marj));
  const minEUR = opts.addon ? 0 : (vars.minSiparis?.currency === 'EUR' ? Number(vars.minSiparis.amount) || 0 : 0);
