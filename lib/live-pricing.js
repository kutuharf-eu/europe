...
async function motorKonfigPrice(cfg, vars, extras, opts = {}) {
  ...
  if (cfg.logo) {
    ...
    if (cfg.logoPrint === 'uv') {
      ...
      const lCost = computeCost({
        ...
        vars: { ...vars, ledAdetSabit: ledAdet }, coeffs: formCoeffs(wM, hM, lg.shape),
      });
      const lProd = lCost.items.filter((i) => i.id !== 'trafo').reduce((a, i) => a + i.tryTotal, 0);
      logoCostTRY = (lProd + iscilikUnitTRY * eq) * fire;
  ...
  if (cfg.cubukLed) {
    ...
    const cCost = computeCost({
      ...
      vars: { ...vars, ledAdetSabit: ledAdet }, coeffs: formCoeffs(lM, hM, 'rect'),
    });
    const cProd = cCost.items.filter((i) => i.id !== 'trafo').reduce((a, i) => a + i.tryTotal, 0);
    cubukCostTRY = (cProd + iscilikUnitTRY * eq) * fire;
