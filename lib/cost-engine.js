...
export function computeCost({ text, h, model, depth, ledType, kromBoyama, uvBaski, vars, coeffs }) {
  ...
  // Montaj gideri (25 Tem 2026 üretici föyü kalibrasyonu): kablo + vida + yapıştırıcı
  // kalemleri parça başı sabit gideri EKSİK kapatıyordu. Föydeki 36 harf (Russo One +
  // Archivo Narrow × D/E/I/K/A/S × 30/40/50 cm) parça başına 180 TL sabit gösteriyor —
  // boydan ve fonttan bağımsız düz (171–186 TL bandı). Motorun kalemleri 147 TL ettiği
  // için aradaki fark bu kalemle kapatılır. Logo/çubuk formlarında bu değer
  // `formMontajTaban` ile ezilir (bkz. live-pricing): oradaki sabit gider LED adediyle
  // büyüyor (166,5 + 2,74 × LED — 11 form satırına ±12 TL içinde oturuyor).
  const montajUnit = toTRY(vars.montajGiderPerHarf, vars, warnings, 'Montaj gideri');
  push('montaj_gideri', 'Montaj gideri', geo.N, 'harf', vars.montajGiderPerHarf, geo.N * montajUnit);
