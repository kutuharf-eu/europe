...
export function letterGeometry({ text, h, coeffs }) {
  ...
  for (const ch of String(text || '').normalize('NFC')) {
    ...
    letters.push({
      ch,
      onYuz: k.alan * h * h,        // m² (net alan; arka yüz ≈ ön yüz)
      cevre: k.cevre * h,           // mt
      genislik: k.genislik * h,     // m
    });
  }
  const sum = (f) => letters.reduce((s, l) => s + f(l), 0);
  return { letters, missing, N: letters.length, toplamOnYuz: sum((l) => l.onYuz), toplamCevre: sum((l) => l.cevre) };
}
...
export function computeCost({ text, h, model, depth, ledType, kromBoyama, uvBaski, vars, coeffs }) {
  ...
  // Yüzeyler (§4 bildirimsel harita)
  const s = model.surfaces;
  if (s.face) m2Item('face', s.face, geo.toplamOnYuz, 'Ön yüz');
  ...
  if (s.back) m2Item('back', s.back, geo.toplamOnYuz, 'Arka yüz');
  if (s.diffuser) m2Item('diffuser', s.diffuser, geo.toplamOnYuz, 'Difüzör');

  // UV baskı (opsiyon) — ana malzemeye EK ayrı kalem (§5.2)
  if (uvBaski && geo.toplamOnYuz > 0) {
    const unit = toTRY(vars.uvBaskiM2, vars, warnings, 'UV baskı');
    push('uv_baski', 'UV baskı', geo.toplamOnYuz, 'm²', vars.uvBaskiM2, geo.toplamOnYuz * unit);
  }

  // ── Adet bazlı kalemler (§5.3) ──
  let ledAdet = 0;
  if (lit) {
    ledAdet = Math.ceil(geo.toplamOnYuz * (vars.ledYogunlugu || 0));
