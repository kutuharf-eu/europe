        ...
        <div className="flex items-center gap-6">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className={`text-[15px] font-semibold whitespace-nowrap ${pathname === l.href ? 'text-accentlite' : 'text-lighttxt hover:text-accentlite'}`}
            >
              {l.label}
            </Link>
          ))}
        ...
        <div className="sm:hidden absolute top-full left-0 right-0 bg-charcoal border-t border-white/10 shadow-dropdown flex flex-col pb-3">
          {links.map((l) => (
            <Link
              ...
              href={l.href}
              onClick={() => setMobileOpen(false)}
              className="px-5 py-3.5 text-[16px] font-medium text-lighttxt hover:text-accentlite"
            >
              {l.label}
            </Link>
          ))}
